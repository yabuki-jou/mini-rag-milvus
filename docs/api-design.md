...
Agent 消息接口内部调用只读工具时遵循以下已确认规则：余额查询一次只查询模型指定的 `leave_type`；申请列表固定返回当前员工最近 20 条并按创建时间倒序；数量、用户、员工和知识库范围均不由客户端或模型覆盖。工具结构化结果由 Agent 应用服务转换为最终回答，当前不新增独立 Tool HTTP API。
